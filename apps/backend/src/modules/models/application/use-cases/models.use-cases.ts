...
export class ModelsUseCases {
  ...
  async listUserModels(userId: string): Promise<OpenCodeModelOption[]> {
    ...
    return connectedProviders.flatMap((providerId) => this.openCodeModels(providerId)
      .map((id) => ({ id, providerId, modelId: id.slice(providerId.length + 1), enabled: enabledModels.includes(id) })))
  }
